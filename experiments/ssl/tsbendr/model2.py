...
class TSBendr_1way(nn.Module):
    ...
    def forward(self, x, masking_ratio=0.0):
        ...
        patch = self.patch(z_sig, z_spe)

        if masking_ratio > 0.0: # 마스킹율이 0이 아니면 SSL(train) mode
            # masking
            masked_patches, _, mask = self.random_mask_patches_except_cls_sep(patch, masking_ratio, self.mask_token)
        ...
        # 마스킹율이 0이면 evaluate mode -> 클래스토큰만 빼서 classification 진행
            ...
            out = self.layer_norm(self.transformer(patch)) # (B, N+1, D)
    ...
    @staticmethod
    def random_mask_patches_except_cls_sep(data, masking_ratio, mask_token):
        ...
        valid_indices[:, 0] = 0  # 0번인덱스인 클래스 토큰과 sep 토큰은 0(False)으로 만들어줌
    ...
    @staticmethod
    def random_masking(x, mask_ratio: float = 0.5):
