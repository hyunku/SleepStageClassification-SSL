...
from experiments.ssl.simclr.data_loader import *
from experiments.ssl.simclr.model import SimCLR
from experiments.ssl.simclr.data_loader import batch_dataloader
...
def get_args():
    ...
    # Dataset
    parser.add_argument('--sampling_rate', default=125)
    parser.add_argument('--base_path', default=os.path.join('..', '..', '..', 'data', 'stage', 'SHHS'))
    ...
    parser.add_argument('--train_lr_rate', default=5e-2, type=float)
    parser.add_argument('--train_batch_size', default=1024, type=int)
    parser.add_argument('--temperature', default=0.1, type=float)
    parser.add_argument('--backbone_name', default='BaseCNN', type=str,
                         choices=['BaseCNN', 'DeepSleepNet', 'CNNEncoder2D'])
...
class Trainer(object):
    def __init__(self, args):
        self.args = args
        self.model = SimCLR(backbone_name=self.args.backbone_name,
        ...
        self.criterion = NTXentLoss(temperature=self.args.temperature).to(device)
