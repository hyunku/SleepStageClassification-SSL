...
from experiments.ssl_sleep.muleeg.model import SleepModel
from experiments.ssl_sleep.muleeg.loss import DiverseLoss
from experiments.evaluation import Evaluation
from experiments.ssl_sleep.muleeg.data_loader import *
...
def get_args():
    ...
    parser.add_argument('--train_lr_rate', default=0.0003, type=float)
    parser.add_argument('--train_batch_size', default=256, type=int)
    parser.add_argument('--muleeg_hidden_dim', default=256, type=int)
    parser.add_argument('--temperature', default=1, type=float)
    parser.add_argument('--intra_temperature', default=10, type=float)
...
class Trainer(object):
    def __init__(self, args):
        self.args = args
        self.model = SleepModel(sampling_rate=args.sampling_rate,
                                hidden_dim=args.muleeg_hidden_dim).to(device)
        self.optimizer = opt.AdamW(self.model.parameters(), lr=self.args.train_lr_rate)
        self.criterion = DiverseLoss(temperature=self.args.temperature,
                                     intra_temperature=self.args.intra_temperature).to(device)
    ...
    def train(self):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for batch in train_dataloader.gather_async(num_async=5):
                ...
                w_t_feats, w_f_feats, w_s_feats, s_t_feats, s_f_feats, s_s_feats = self.model(x1, x2)
                loss = self.criterion(w_t_feats, w_f_feats, w_s_feats, s_t_feats, s_f_feats, s_s_feats)
    ...
    def compute_evaluation(self):
        ...
        bf_pred, bf_real = evaluation.fine_tuning(ft_paths=self.ft_paths,
                                                  ...
                                                  frozen_layers=['time_model', 'attention', 'spectral_model'])
    ...
    def data_paths(self):
        ...
        train_paths, ft_paths, eval_paths = paths['train_paths'][:5], paths['ft_paths'], paths['eval_paths']
