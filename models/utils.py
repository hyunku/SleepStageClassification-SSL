...
def get_backbone_model(model_name, parameters):
    ...
    from models.backbone import ZZLet
    from models.backbone import CNNEncoder2D_SLEEP
    from models.backbone import BENDREncoder

    if model_name == 'BaseCNN':
        backbone = BaseCNN(**parameters)
        return backbone
    if model_name == 'ZZNet':
        backbone = ZZLet(**parameters)
        return backbone
    if model_name == 'CNNEncoder2D_SLEEP':
        backbone = CNNEncoder2D_SLEEP(**parameters)
        return backbone
    if model_name == 'BENDREncoder':
        backbone = BENDREncoder(**parameters)
        return backbone
...
def get_backbone_parameter(model_name, sampling_rate: float):
    if model_name == 'BaseCNN':
        ...
        return parameter
    if model_name == 'ZZNet':
        parameter = {'sampling_rate': sampling_rate, 'stride': 3, 'cnn_dropout': 0.5, 'rnn_dropout': 0.5,
                     'rnn_dim': 100, 'rnn_layers': 1}
        return parameter
    if model_name == 'CNNEncoder2D_SLEEP':
        parameter = {'n_dim': 1024, 'sampling_rate': sampling_rate}
        return parameter
    if model_name == 'BENDREncoder':
